...
void DisplayLib::addList(int xPos, int yPos, int itemSpacing, bool downwards, std::string textList[], int listSize, bool identifier) {
  if (downwards) {
    int yyPos = yPos;
    for (int i = 0; i < listSize; i++) {
      if(identifier){
        addText(textList[i].c_str(), xPos, yyPos, textList[i++].c_str());
      } else {
